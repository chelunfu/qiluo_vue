    ...
    <ElForm>
      <ElFormItem>
        <ElRadio v-model="groupV" :label="DayGroup.EVERY" :key="cronTypeV + DayGroup.EVERY + ''">
          <span class="font-bold">
            {{ cronTypeV === CronType.SECOND ? t('vueCron.t.second') : t('vueCron.t.minute') }}
          </span>
      ...
      <ElFormItem>
        <ElRadio v-model="groupV" :label="DayGroup.CYCLE" :key="cronTypeV + DayGroup.CYCLE + ''">
          ...
          <span class="m-l-10px">
            {{ t('vueCron.from') }}
            <ElInputNumber v-model="cycle.from" :min="0" :max="59" />
          </span>
          {{ t('vueCron.to') }}
          <ElInputNumber v-model="cycle.to" :min="cycle.from + 1" :max="59" />
          {{ cronTypeV === CronType.SECOND ? t('vueCron.secondTitle') : t('vueCron.minuteTitle') }}
        </ElRadio>
      </ElFormItem>
      <ElFormItem>
        <ElRadio
        ...
        >
          ...
          <ElInputNumber v-model="cycle.every" :min="1" :max="59 - cycle.from" />
          {{
            cronTypeV === CronType.SECOND ? t('vueCron.secondTitle2') : t('vueCron.minuteTitle2')
          }}
