    ...
    <ElForm>
      ...
      <ElFormItem>
        <ElRadio v-model="groupV" :label="DayGroup.CYCLE" :key="cronTypeV + DayGroup.CYCLE + ''">
          ...
          <span class="m-l-10px">
            {{ t('vueCron.from') }}
            <ElInputNumber v-model="cycle.from" :min="0" :max="23" />
          </span>
          {{ t('vueCron.to') }}
          <ElInputNumber v-model="cycle.to" :min="cycle.from + 1" :max="23" />
          {{ t('vueCron.hourTitle') }}
        </ElRadio>
      </ElFormItem>
      <ElFormItem>
        ...
        >
          ...
          <ElInputNumber v-model="cycle.every" :min="1" :max="23 - cycle.from" />
