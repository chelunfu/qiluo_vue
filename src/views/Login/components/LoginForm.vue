...
import { PostLogin, GetAdminRole } from '@/api/login'
import { useAppStore } from '@/store/modules/app'
import { usePermissionStore } from '@/store/modules/permission'
import { useRouter } from 'vue-router'
import type { RouteLocationNormalizedLoaded, RouteRecordRaw } from 'vue-router'
import { UserLoginParamType, CodeData } from '@/api/login/types'
...
const getCode = async () => {
  ...
  await captchaStore.refreshCaptcha()
  captchaData.value = captchaStore.captchaData
        ...
        default: (formData) => {
          return (
            <div class="w-[100%] flex">
              <ElInput v-model={formData.captcha} placeholder={t('login.codePlaceholder')} />
              <div onClick={getCode}>
                <ElImage src={captchaData.value?.img || ''} class="ml-10px" />
              </div>
            </div>
          )
        }
...
const signIn = async () => {
  ...
  await formRef?.validate(async (isValid) => {
    if (isValid) {
      loading.value = true
      const formData = await getFormData<UserLoginParamType>()
      if (captchaData.value) {
        formData.captchaid = captchaData.value.uuid
      } else {
        formData.captchaid = ''
      }
      formData.client_id = captchaStore.clientId
      try {
...
const getRole = async () => {
  ...
  if (res) {
    ...
    permissionStore.setIsAddRouters(true)
    const permission = await role_permission_list()
